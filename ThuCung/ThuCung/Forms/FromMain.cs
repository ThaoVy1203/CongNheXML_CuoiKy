    ...
    Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\..\data\ThuCung.xml"));
        ...
        private void LoadDataFromXml()
        {
            try
            {
                ...
                // Xóa tất cả các quan hệ tự động để tránh lỗi constraint
                if (dsPetShop.Relations.Count > 0)
                {
                    for (int i = dsPetShop.Relations.Count - 1; i >= 0; i--)
                    {
                        dsPetShop.Relations.RemoveAt(i);
                    }
                }
                
                // Xóa tất cả các constraint trong các bảng
                ...
                using (SqlConnection conn = DatabaseHelper.DatabaseHelper.GetConnection())
                {
                    ...
                    using (SqlTransaction tran = conn.BeginTransaction())
                    {
                        try
                        {
                            ...
                            // 4) tbThuCung - BỎ QUA cột tenLoai vì nó không có trong SQL
                            InsertWithIdentity("tbThuCung", _ =>
                            {
                              ...
                              VALUES (@id,@ten,@tuoi,@gt,@ms,@cn,@giong,@tc,@anh,@gia,@idLoai)", conn, tran))
                                ...
                                new SqlParameter("@idLoai", SqlDbType.Int)
                                    ...
                                    {
                                        // Bỏ qua các element không cần thiết như tenLoai (chỉ dùng để hiển thị)
                                        var idElement = tc.Element("idThuCung");
                                        if (idElement == null) continue;

                                        cmd.Parameters["@id"].Value = int.Parse(idElement.Value);
                                        ...
                                        cmd.Parameters["@idLoai"].Value = int.Parse(tc.Element("idLoai")?.Value ?? "1");
                                        cmd.ExecuteNonQuery();
                                    }
                                }
                            });
                            ...
                            // 6) tbChiTietHoaDon - Kiểm tra foreign key trước khi INSERT
                            InsertWithIdentity("tbChiTietHoaDon", _ =>
                            {
                                // Lấy danh sách ID thú cưng và hóa đơn có trong XML
                                var validThuCungIds = doc.Root.Elements("ThuCung")
                                    .Select(tc => int.Parse(tc.Element("idThuCung").Value))
                                    .ToHashSet();
                                var validHoaDonIds = doc.Root.Elements("HoaDon")
                                    .Select(hd => int.Parse(hd.Element("idHoaDon").Value))
                                    .ToHashSet();
                              ...
                              VALUES (@id,@hd,@tc,@sl,@dg,@tt)", conn, tran))
                                ...
                                new SqlParameter("@tt", SqlDbType.Decimal){ Precision = 18, Scale = 2 }
                                    ...
                                    {
                                        int idThuCung = int.Parse(ct.Element("idThuCung").Value);
                                        int idHoaDon = int.Parse(ct.Element("idHoaDon").Value);

                                        // Bỏ qua nếu idThuCung hoặc idHoaDon không tồn tại
                                        if (!validThuCungIds.Contains(idThuCung) || !validHoaDonIds.Contains(idHoaDon))
                                        {
                                            continue;
                                        }

                                        cmd.Parameters["@id"].Value = int.Parse(ct.Element("idChiTiet").Value);
                                        cmd.Parameters["@hd"].Value = idHoaDon;
                                        cmd.Parameters["@tc"].Value = idThuCung;
        ...
        {
            ShowPage();
            LoadDataFromXml();  // Reload dữ liệu từ XML

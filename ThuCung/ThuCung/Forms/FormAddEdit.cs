     ...
     Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, @"..\..\data\ThuCung.xml"));
        ...
        private void LoadPetData()
        {
            try
            {
                ...
                if (!dtThuCung.Columns.Contains("idThuCung"))
                {
                    MessageBox.Show("Cấu trúc bảng ThuCung không hợp lệ! Thiếu cột idThuCung.", "Lỗi");
                    return;
                }

                // Tìm thú cưng theo ID bằng cách duyệt qua từng dòng
                DataRow row = null;
                foreach (DataRow r in dtThuCung.Rows)
                {
                    if (r["idThuCung"] != DBNull.Value && Convert.ToInt32(r["idThuCung"]) == petId)
                    {
                        row = r;
                        break;
                    }
                ...
                if (row == null)
                {
                    MessageBox.Show($"Không tìm thấy thú cưng có ID = {petId}", "Lỗi");
                    return;
                }
                
                // Fill dữ liệu vào các controls
                txtTenThuCung.Text = row["tenThuCung"]?.ToString() ?? "";
                txtTuoi.Text = row["tuoi"]?.ToString() ?? "0";
                txtGiongThuCung.Text = row["giongThuCung"]?.ToString() ?? "";
                
                // Set giới tính - đảm bảo combobox có items
                if (cboGioiTinh.Items.Count == 0)
                {
                    cboGioiTinh.Items.AddRange(new object[] { "Đực", "Cái" });
                }
                string gioiTinh = row["gioiTinh"]?.ToString() ?? "Đực";
                int gioiTinhIndex = cboGioiTinh.Items.IndexOf(gioiTinh);
                if (gioiTinhIndex >= 0)
                {
                    cboGioiTinh.SelectedIndex = gioiTinhIndex;
                }
                
                txtMauSac.Text = row["mauSac"]?.ToString() ?? "";
                txtCanNang.Text = row["canNang"]?.ToString() ?? "0";
                txtTieuChuanCanNang.Text = row["tieuChuanCanNang"]?.ToString() ?? "0";
                txtGiaBan.Text = row["giaBan"]?.ToString() ?? "0";
                
                // Set loại thú cưng
                if (row["idLoai"] != DBNull.Value)
                {
                    cboLoaiThuCung.SelectedValue = Convert.ToInt32(row["idLoai"]);
                }
